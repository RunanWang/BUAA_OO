...
public class QueryList {
    ...
    /*@ public normal_behavior
      ...
      @ assignable lastTime;
      @ ensures (req.targetFloor < highLevel && req.targetFloor > lowLevel) ==> (\result == true);
      @ ensures (\result == true) ==> queue.contains(req)==true && queue.getIndex(req)==queue.size()-1;
      @ ensures (\result == false) ==> queue.contains(req) ==false;
      @ ensures queue.contains(req)==true ==> lastTime == req.getTime();
      @ also
      @ public exceptional_behavior
      @ requires req.queryTime < lastTime;
    ...
    public Query removeQuery(int index) throws InvalidRemoveException {
        //请依据规格补充此处缺少的代码
        if (index < 0) {
            throw new InvalidRemoveException("index less than zero");
        }
        try {
            queue.get(index);
        } catch (ArrayIndexOutOfBoundsException e) {
            throw new InvalidRemoveException("index larger than bound");
        }
