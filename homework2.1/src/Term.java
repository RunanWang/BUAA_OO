...
class Term {
    ...
    private ArrayList<Factor> termDerRes = new ArrayList<>();
    ...
    public ArrayList<Term> termDer(){
        Factor item;
        ArrayList<Factor> facDerRes;
        ArrayList<Factor> termRes = new ArrayList<>();
        int i = 0;
        int j = 0;
        for (i = 0; i < termList.size(); i++) {
            item = termList.get(i);
            termRes = (ArrayList<Factor>) termList.clone();
            termRes.remove(i);
            facDerRes = item.facDer();
            for(j=0;j<facDerRes.size();j++){
