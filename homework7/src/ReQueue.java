...
public class ReQueue {
    private Vector<Request> queue = new Vector<>();
    private Vector<Request> runQueue = new Vector<>();
     ...
     * */
    public synchronized void addQueue(Request r) {
        queue.add(r);
        total++;
        if (total == 1) {
            notifyAll();
    ...
    public synchronized int getFirstRequest(int i) {
        ...
        return queue.elementAt(i).getRequestNow();
    }
     ...
     * 设置为0时是ALS
     * */
    public synchronized Request getFirstRe() {
        while (isEmpty()) {
            try {
                wait();
                //唤醒后如果需求输入结束则退出
                //System.out.println(runQueue.isEmpty());
                if (isEnd() && runQueue.isEmpty()) {
                    break;
                }
            } catch (Exception e) {
                e.printStackTrace();
     ...
     * 说明该项进电梯了
     * */
    public synchronized Request outQueue(int index) {
     ...
     * 说明该项被Monitor处理过了
     * */
    public synchronized void outTotalQueue(int i) {
    ...
    public synchronized void outRunQueue(Request r) {
        runQueue.remove(runQueue.indexOf(r));
